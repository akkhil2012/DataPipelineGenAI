...
- **Typer CLI** for running the pipeline end-to-end and writing aggregated insights as JSON.
- **Extensibility hooks** so that real LLM providers can be injected without changing orchestration logic.

## Getting Started

Install the package in editable mode and run the CLI against the bundled sample data set:

```bash
pip install -e .
python -m datapipeline_genai.cli run --input data/sample_calls.csv --output build/report.json
```
